			...
			Global.workerPool.runTask(WorkerManager.GREYHOUND, function(e)
			{
				...
				for(var property in buffers)
				{
					...
					else if(parseInt(property) === PointAttributeNames.COLOR_PACKED)
					{
						...
						geometry.setAttribute("color", new THREE.BufferAttribute(Global.denormalizeUint8Array(new Uint8Array(buffer)), 4));
					}
	...
	class LASLAZBatcher
	{
		...
		push(data)
		{
			...
			worker.onmessage = function(e)
			{
				...
				geometry.setAttribute("color", new THREE.BufferAttribute(Global.denormalizeUint8Array(colors), 4));
		...
		parse(node, buffer)
		{
			...
			worker.onmessage = function(e)
			{
				...
				if(e.data.color)
				{
					...
					g.setAttribute("color", new THREE.BufferAttribute(Global.denormalizeUint8Array(color), 4));
				}
				if(e.data.intensity)
				{
	...
	class EptLazBatcher
	{
		...
		push(las)
		{
			...
			worker.onmessage = (e) =>
			{
				...
				g.setAttribute("color", new THREE.BufferAttribute(Global.denormalizeUint8Array(colors), 4));
		...
		load()
		{
			...
			xhr.onload = function()
			{
				try
				{
					...
					geometry.setAttribute("color", new THREE.BufferAttribute(Global.denormalizeUint8Array(color), 4));
					geometry.setAttribute("intensity", new THREE.BufferAttribute(intensities, 1));
					geometry.setAttribute("classification", new THREE.BufferAttribute(classifications, 1));
					{
